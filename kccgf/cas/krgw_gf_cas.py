...
'''
This code is hacked for performing CAS G0W0 calculations.
...
def kernel(gw, gfomega, mo_energy, mo_coeff, orbs=None,
    ...
    ### Hacked for CAS GW@HF vk and v_mf ###
    if gw.vk is not None:
        v_mf = gw.vk
        vk = gw.vk
    else:
        v_mf = np.zeros((nkpts, nmo, nmo),dtype=complex)
        vk = np.zeros((nkpts, nmo, nmo),dtype=complex)

    if gw._scf.exxdiv == 'ewald':
        from pyscf.pbc import tools
        madelung = tools.pbc.madelung(gw.mol, [gw.kpts])
        for k in range(nkpts):
            for i in range(nocc):
                vk[k][i,i] = vk[k][i,i] + madelung
    if gw._scf.exxdiv is not None and gw._scf.exxdiv != 'ewald':
